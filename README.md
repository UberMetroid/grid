...
|---|---|---|
| `PORT` | The port number the backend HTTP server will bind to. | `4405` |
| `GRID_PIN` | Optional PIN to restrict access to the application interface. | (None) |
| `GRID_DATA_DIR` / `DATA_DIR` | Path to the directory where application data and database files are stored. | `data` / `/config` |
| `GRID_ALLOWED_ORIGINS` / `ALLOWED_ORIGINS` | Comma-separated list of allowed CORS origins, or `*` to allow all. | `*` |
